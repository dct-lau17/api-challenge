...
const createFile = () => {
  ...
  if (!fs.existsSync(path)) {
    createDir(path);
    sendToTask3("create directory", folder[folder.length - 1]);
  ...
  if (!fs.existsSync(filePath)) {
    ...
    sendToTask3("create file", folder[folder.length - 1], [fileName]);
...
const deleteFiles = function (dir) {
  ...
  files.forEach((file) => {
    ...
    if (fs.statSync(path).isDirectory()) {
      ...
      if (fs.readdirSync(path).length === 0) {
        ...
        sendToTask3("delete directory", path2);
      ...
      folder = pathSplit[pathSplit.length - 2];
  ...
  if((fileList.length  && folder != '') ){
    sendToTask3("delete files", folder, fileList)
  }
...
const sendToTask3 = (action, folder, files = []) => {
  ...
  }).then(data=>console.log('status: ' + data.status + ' response data: ' + data.data)).catch(err=>console.log(err))
};
